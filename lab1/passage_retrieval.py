...
def initialize_data(documents:list[dict], questions:list[dict]) -> None:
...
if __name__ == "__main__":
    ...
    doc_vectors= model.encode([doc["text"] for doc in documents])
    query_vectors= model.encode([q["question"] for q in queries])

    results= util.semantic_search(query_vectors, doc_vectors)
    for i in range(len(query_vectors)):
        print(queries[i])
        docindex=results[i][0]["corpus_id"]
